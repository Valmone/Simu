function simulation() {
	...
	//---------------------------------Classes----------------------------------//
	//--------------------------------------------------------------------------//
	function Attaquant(techno_arme, techno_bouclier, techno_protection) {
		this.pt = null;
		this.gt = null;
		this.cle = null;
		this.clo = null;
	...
	function Defenseur(techno_arme, techno_bouclier, techno_protection) {
		this.pt = null;
		this.gt = null;
		this.cle = null;
		this.clo = null;
	...
	function Vaisseau(nbr, type, arme, bouclier, protection) {
		this.nbr = nbr;
		this.type = type;
		this.arme = arme;
		...
		this.arme_total = null;
		this.bouclier_total = null;
		this.protection_total = null;
		this.bouclier_initial_total = null;
		this.protection_initial_total = null;
	};
	...
	function insert(total, qui) {				// on parametres chaques vaisseau
		...
		for (var i = 0; i <= 13; i++) {
			...
			total[type[i]].arme_total = total[type[i]].nbr *total[type[i]].arme;
			total[type[i]].bouclier_total = total[type[i]].nbr *total[type[i]].bouclier;
			total[type[i]].protection_total = total[type[i]].nbr *total[type[i]].protection;
			total[type[i]].bouclier_initial_total = total[type[i]].bouclier_total;
			total[type[i]].protection_initial_total = total[type[i]].protection_total;
	...
	function recharge_bouclier(total) {			// on recharge les boucliers à chaque tour
		var type = new Array("pt", "gt", "cle", "clo", "cro", "vb", "vc", "cyclo", "sonde", "sat", "bb", "dd", "rip", "traq");
		for (var i = 0; i < 13; i++) {
			total[type[i]].bouclier_total = total[type[i]].bouclier_initial_total;
	...
	function dommages(predateur, proie) {		// on attaque le bouclier, et la coque si c'est possible
		if (predateur.arme_total < (proie.bouclier_total /100)) {
			// -- on ne fait rien
		} else {
			if (predateur.arme_total < proie.bouclier_total) {
				proie.bouclier_total = proie.bouclier_total - predateur.arme_total;
			} else {
				proie.protection_total = proie.protection_total - (predateur.arme_total - proie.bouclier_total);
				proie.bouclier_total = 0;				
			}
			if (proie.protection_total < 1) {
				proie.protection_total = -1;
			} else {}
		}
		return proie;
	};
	...
	function rapidfire(predateur, proie) {//, nbr) {		// on calcul si il y a ou pas RF
		var type = new Array("pt", "gt", "cle", "clo", "cro", "vb", "vc", "cyclo", "sonde", "sat", "bb", "dd", "rip", "traq"),
			proba = RapidFire[type.indexOf(predateur)][type.indexOf(proie)] *10,
	...
	function combat(predateur, proie) {			// chaque vaisseau attaque un vaisseau aléatoirement
		...
		for (var i = 0; i <= 13; i++) {
			...
			if (proie[type[i]].nbr > 0) {
				diff_proie++;
				liste_defenseur[liste_defenseur.length] = i;
			} else {}
		}

		for (var i = 0; i <= 13; i++) {
			if ((liste_attaquant.indexOf(i) >= 0) && (predateur[type[i]].nbr > 0)) {
				do {
					var id_proie = Math.floor(Math.random() *(diff_proie)); 			// selection de la cible aléatoire
					proie[type[liste_defenseur[id_proie]]] = dommages(predateur[type[liste_attaquant[i]]], proie[type[liste_defenseur[id_proie]]]);
					if (proie[type[liste_defenseur[id_proie]]].protection_total < (proie[type[liste_defenseur[id_proie]]].protection_initial_total) *0.7) {
						proie[type[liste_defenseur[id_proie]]] = proba_destruction(proie[type[liste_defenseur[id_proie]]], ((proie[type[liste_defenseur[id_proie]]].protection_total *100) / proie[type[liste_defenseur[id_proie]]].protection_initial_total));
					} else {}
				} while(rapidfire(predateur[type[liste_attaquant[i]]].type, proie[type[liste_defenseur[id_proie]]].type) === "oui");
			} else {}
		}
		return proie;
	};
	...
	function suppression_vaisso(total) {		// on supprime un vaisseau si li est détruit
		...
		for (var i = 0; i <= 13; i++) {
			if (total[type[i]].nbr > 0) {
				if (total[type[i]].protection_total === -1) {					
					total[type[i]].total = total[type[i]].total - total[type[i]].nbr;
					total.total = total.total - total[type[i]].nbr;
					total[type[i]].nbr = 0;
					i--;
				} else {
					if (total[type[i]].protection_total < total[type[i]].protection_initial_total) {
						var sert_a_tout = total[type[i]].protection_initial_total - total[type[i]].protection_total;
						sert_a_tout = sert_a_tout / total[type[i]].protection;
						if (sert_a_tout >= 1) {
	...
	//-------------------------------Récupération-------------------------------//
	//--------------------------------------------------------------------------//
	var attaquant = new Attaquant(verification(document.getElementsByName("tech_a_0")[0].value),
			verification(document.getElementsByName("tech_a_1")[0].value),
			verification(document.getElementsByName("tech_a_2")[0].value)
		),
		defenseur = new Defenseur(verification(document.getElementsByName("tech_d_0")[0].value),
